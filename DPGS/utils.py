...
def get_one_hot(y_s):
    """Convert task labels to one-hot encodings."""
...
def get_logs_path(model_path, method, shot):
    """Build a unique log path for an evaluation setting."""
...
def get_features(model, samples):
    """Extract and normalize model features."""
...
def get_loss(logits_s, logits_q, labels_s, lambdaa):
    """Compute the supervised and information-gain loss."""
    Q = logits_q.softmax(2)
    y_s_one_hot = get_one_hot(labels_s)
    ce_sup = - (y_s_one_hot * torch.log(logits_s.softmax(2) + 1e-12)).sum(2).mean(1)
    ent_q = get_entropy(Q)
...
def get_mi(probs):
    """Compute mutual information from class probabilities."""
...
def get_cond_entropy(probs):
    """Compute mean conditional entropy."""
    q_cond_ent = - (probs * torch.log(probs + 1e-12)).sum(2).mean(1, keepdim=True)
    return q_cond_ent

def get_metric(metric_type):
    """Return the requested pairwise distance function."""
    METRICS = {
...
class AverageMeter(object):
    """Track the current value, sum, count, and running average."""
...
def setup_logger(filepath):
    """Create a file logger without duplicate handlers."""
...
def warp_tqdm(data_loader, disable_tqdm):
    """Optionally wrap a data loader with a progress bar."""
    if disable_tqdm:
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar', folder='result/default'):
    """Save a checkpoint and optionally mark it as the best model."""
...
def load_checkpoint(model, model_path, type='best'):
    """Load the best or latest checkpoint into a model."""
    if type == 'best':
...
def _decode_cfg_value(v):
    """Decode a string configuration value into a Python value."""
...
def _check_and_coerce_cfg_value_type(replacement, original, key, full_key):
    """Validate and coerce an overridden configuration value."""
...
def load_cfg_from_cfg_file(file: str):
    """Load and flatten a YAML configuration into a CfgNode."""
    cfg = {}
...
def merge_cfg_from_list(cfg: CfgNode,
                        cfg_list: List[str]):
    """Apply command-line key-value overrides to a configuration."""
...
class Logger:
    """Write formatted log messages to both a file and the console."""
...
def make_log_dir(log_path, dataset, backbone, method):
    """Create the directory used for evaluation logs."""
...
def get_log_file(log_path, dataset, backbone, method):
    """Return the next available log filename."""
...
def extract_features(model, support, query):
    """Extract support and query features for each task."""
    model.eval()
    with torch.no_grad():
        outputs_s = []
        outputs_q = []

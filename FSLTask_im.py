...
features_dict['wrn_s2m2'] = {
    "mini": "./features/wrn_s2m2/mini/novel.plk",
...
# Runtime defaults; eval.py overrides _maxRuns and _alpha.
_cacheDir = "./cache"
_maxRuns = 1000
_min_examples = -1
_alpha = 2
...
def get_dirichlet_query_dist(alpha, n_tasks, n_ways, q_shots):
    """Sample per-class query counts from a Dirichlet distribution."""
...
def convert_prob_to_samples(prob, q_shot):
    """Convert class probabilities to integer counts with a fixed total."""
    prob = prob * q_shot
...
def _load_pickle(file):
    """Load feature vectors and labels from a pickle file."""
...
def loadDataSet(backbone, dsname):
    """Load and group features by class for task generation."""
...
def GenerateRun(iRun, cfg, regenRState=True, generate=True):
    """Generate one balanced or Dirichlet few-shot task."""
...
def ClassesInRun(iRun, cfg):
    """Return the class indices selected for one task."""
...
def setRandomStates(cfg):
    """Load or generate reproducible random states for all tasks."""
...
def GenerateRunSet(start=None, end=None, cfg=None):
    """Generate a batch of few-shot tasks."""
